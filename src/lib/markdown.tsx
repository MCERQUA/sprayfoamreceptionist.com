...
function renderInline(text: string, keyPrefix: string): ReactNode[] {
  ...
  return parts.map((part, i) => {
    ...
    if (linkMatch) {
      return (
        <Link key={key} href={linkMatch[2]} className="text-amber underline hover:text-amberDark">
          {linkMatch[1]}
        </Link>
      );
    }
...
export function renderMarkdown(md: string): ReactNode[] {
  ...
  return blocks.map((block, i) => {
    ...
    if (block.startsWith("> ")) {
      return (
        <blockquote key={key} className="border-l-4 border-amber pl-5 italic text-inkSoft leading-relaxed my-6">
