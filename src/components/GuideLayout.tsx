...
export default function GuideLayout({ content }: { content: GuideContent }) {
  const guide = getGuide(content.slug)!;
    ...
    <>
      <section className="relative bg-paperRaised">
        <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-16 sm:py-20">
          <p className="text-amber font-semibold text-sm uppercase tracking-wide mb-3">Guide</p>
      ...
      <div className="relative w-full h-64 sm:h-80 lg:h-96">
        <Image src={content.image} alt={guide.h1} fill className="object-cover" priority />
      </div>
      ...
      {related.length > 0 && (
        <section className="bg-paperRaised">
          <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-14">
            <h2 className="font-head text-2xl font-bold text-ink mb-6">Related Guides</h2>
            <div className="grid sm:grid-cols-2 gap-4">
              {related.map((g) => (
                ...
                >
                  <span className="font-medium text-ink group-hover:text-amber transition-colors">
                    {g.navLabel}
                  </span>
